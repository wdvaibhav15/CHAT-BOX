...
// send notifications for unreen messages
// const sendNotificationOfUnseenMessages = inngest.createFunction(
//   {id: "send-unreen-messages-notifications"},
//   {cron: "Tz=America/New-York 0 1 * * *"},//every day at 1 am
//   async({step}) => {
...
//           <p>Click <a href="${process.env.FRONTEND_URL}/messages" style="color: #10b981; text-decoration: none;">here</a> to view them</p>
//           <br/>
...
//   }
// )
// Send notifications for unseen messages
export const sendNotificationOfUnseenMessages = inngest.createFunction(
  ...
  async ({ step }) => {
    const result = await step.run("send-unseen-message-emails", async () => {
      ...
      for (const message of messages) {
        const receiverId = message.to_user_id?._id?.toString();
      ...
      for (const userId of Object.keys(unseenCount)) {
        ...
        const body = `
          <div style="font-family: Arial, sans-serif; padding: 20px;">
            ...
            <p>
              You have ${unseenCount[userId]} unseen
              ${unseenCount[userId] === 1 ? "message" : "messages"}.
            </p>
      ...
      return {
        sentEmails,
        usersWithUnseenMessages: Object.keys(unseenCount).length,
      };
    });

    return {
      message: "Unseen-message notifications processed.",
      ...result,
    };
  },
);
